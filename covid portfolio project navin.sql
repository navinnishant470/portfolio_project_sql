...
select location,date,total_cases,population, (total_cases/population*100) as percent_population_infected from covid where location='India'

  # 1. global numbers i.e. across the world
  select date, sum(new_cases),sum(new_deaths),( sum(new_deaths)/sum(new_cases))*100 as death_percent from covid group by date
   select  sum(new_cases) as total_cases,sum(new_deaths) as total_deaths,( sum(new_deaths)/sum(new_cases))*100 as death_percent from covid 
   
#  2.locationwise(continent) total deaths , Europian union is a part of europe
select location, sum( new_deaths ) as total_deaths from covid where continent is null and 
location not in ('World', 'High Income', 'Upper middle income', 'Lower middle income','Low income', 'European Union','International')
group by location  order by total_deaths desc

#3.looking at countries with highest infection rate compared to population
select location,population ,max(total_cases) as 'highest infection count', max(total_cases/population*100) as percent_population_infected
 from covid where
location not in (continent,'World', 'High Income', 'Upper middle income', 'Lower middle income','Low income', 'European Union','International')
 group by location, population order by percent_population_infected desc
 
 #4.looking at countries with highest infection rate compared to population grouped by date
select location,population,date ,max(total_cases) as 'highest infection count', max(total_cases/population*100) as percent_population_infected
 from covid group by location, population,date order by percent_population_infected desc
...
select continent, location,  population, max(rolling_ppl_vaccinated)/population*100 as total_vaccinated from PopvsVac group by location
 ...
 #creating view for later data visualisation if needed
